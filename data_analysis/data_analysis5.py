...
for col_name in string_cols:
    ...
    string_df = indexer.fit(df).transform(df)

    # 计算与label的相关性（使用编码后的值）
    corr_value = string_df.stat.corr(f"{col_name}_indexed", "label")
    print(f"{col_name} 与 label 的相关性: {corr_value:.4f}")

    # 将索引列转换为向量列
    vectorize = udf(lambda x: Vectors.dense([x]), VectorUDT())
    string_df = string_df.withColumn(f"{col_name}_vector", vectorize(f"{col_name}_indexed"))

    # 卡方检验（使用向量列）
    chi_result = ChiSquareTest.test(string_df, f"{col_name}_vector", "label").head()
...
for col_name in bool_cols:
    # 将布尔值转换为整数(0/1)
    bool_df = df.withColumn(f"{col_name}_int", col(col_name).cast("integer"))

    # 过滤掉包含NaN或null的行
    filtered_df = bool_df.filter(
        col(f"{col_name}_int").isNotNull() &
        col("label").isNotNull()
    )
    ...
    # 使用VectorAssembler创建向量列，handleInvalid="skip"会过滤掉null
...
for col_name in cat_cols:
    print(f"\n{col_name} 的分布（含null）:")
    distribution = df.groupBy(col_name).count().collect()
