...
from pyspark.sql import functions as F  # 使用别名避免变量名冲突
...
for col_name in numeric_cols:
    ...
    corr_results.append((col_name, corr_value))

# 输出表格头部
print("{:<15} {:<20}".format("变量名", "皮尔逊相关系数"))
print("-" * 35)
# 输出表格内容
for col, corr in corr_results:
    print("{:<15} {:<20.4f}".format(col, corr))
...
for col_name in cat_cols:
    ...
    if col_name in bool_cols or col_name == 'category':
        ...
        chi_results.append((col_name, p_value))
...
# 输出卡方检验结果表格
print("\n{:<15} {:<20}".format("变量名", "卡方检验p值"))
print("-" * 35)
for col, p_val in chi_results:
    print("{:<15} {:<20.4f}".format(col, p_val))
...
for col_name in bool_cols:
    # 将布尔值转换为整数(0/1)
    df = df.withColumn(f"{col_name}_int", F.col(col_name).cast("integer"))
