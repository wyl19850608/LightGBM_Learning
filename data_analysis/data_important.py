...
# 计算每列和label的相关性，计算时过滤掉空值
correlation_results = {}

for col in numeric_cols:
    # 过滤当前列和label列的空值
    valid_data = df.select(col, 'label').dropna()

    # 计算相关系数
    correlation = valid_data.stat.corr(col, 'label')
...
print("各数值列与label的相关性:")
print(correlation_df)

# 保存结果到CSV
correlation_df.to_csv("feature_label_correlations.csv", index_label='Feature')
