...
class DataProcessor:
    ...
    def transform(self, df, numeric_cols, categorical_cols, bool_cols, string_cols, label_col=None):
        ...
        # 处理字符串特征
        for col in string_cols:
            if col in self.string_categories:
                df[col] = df[col].astype(str)
                # 处理未知类别
        ...
        # 处理标签列
        if label_col and label_col in self.label_encoders:
...
class DeepFM:
    def __init__(self, num_numeric, field_dims, embedding_dim=8, hidden_units=[128, 64],
        ...
        参数:
            num_numeric: 连续特征数量
            field_dims: 每个需要嵌入的特征的维度列表
            embedding_dim: 嵌入层维度
            hidden_units: 深度网络隐藏层单元数列表
...
def main():
    ...
    data = {
        ...
        'num_feature2': np.random.normal(5, 2, n_samples),

        # 非连续型数字特征
        'cat_feature1': np.random.randint(0, 5, n_samples),
        'cat_feature2': np.random.randint(0, 3, n_samples),

        # 布尔型特征
        'bool_feature1': np.random.randint(0, 2, n_samples).astype(bool),
        'bool_feature2': np.random.randint(0, 2, n_samples).astype(bool),

        # 字符串特征
        'str_feature1': np.random.choice(['apple', 'banana', 'cherry', 'date'], n_samples),
        'str_feature2': np.random.choice(['red', 'green', 'blue', 'yellow', 'black'], n_samples),
    ...
    # 将训练集和测试集重新拆分为特征列表
    # 顺序：numeric -> categorical -> bool -> string
