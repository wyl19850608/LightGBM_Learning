...
from sklearn.feature_selection import chi2, SelectKBest, f_classif  # 添加了f_classif
...
class FeaturePreprocessor:
    ...
    def __init__(self,
                 ...
                 numeric_categorical_features: List[str] = None,  # 新增：数字分类特征
        ...
        参数:
            ...
            numeric_categorical_features: 数字分类特征列表
        ...
        self.numeric_categorical_features = numeric_categorical_features or []  # 新增

        # 合并所有类别型特征，包括数字分类特征
        self.all_categorical = (self.categorical_features +
                                self.boolean_features +
                                self.string_features +
                                self.numeric_categorical_features)  # 更新
    ...
    def _encode_categorical_features(self, df: pd.DataFrame) -> None:
        ...
        for col in self.all_categorical:
            if col in df.columns:
                # 确保数字分类特征被视为对象类型
                if col in self.numeric_categorical_features:
...
def main():
    ...
    # 新增：3个数字分类特征（值为1,2,3的分类）
    for i in range(3):
        data[f'num_cat_{i}'] = np.random.choice([1, 2, 3, None], n_samples)
    ...
    # 定义特征类型
    continuous_features = [f'num_{i}' for i in range(10)]
    categorical_features = [f'cat_{i}' for i in range(10)]
    boolean_features = [f'bool_{i}' for i in range(5)]
    string_features = [f'str_{i}' for i in range(5)]
    numeric_categorical_features = [f'num_cat_{i}' for i in range(3)]  # 新增
    ...
    preprocessor = FeaturePreprocessor(
        ...
        numeric_categorical_features=numeric_categorical_features,  # 新增
