...
class FeatureProcessor:
    ...
    def save_config(self, path):
        ...
        bin_rules = {}
        for feature, binner in self.config['bin_discretizers'].items():
            # KBinsDiscretizer的bin_edges_属性存储分箱边界
            # 对于单特征，bin_edges_[0]即为该特征的分箱边界数组
            bin_edges = binner.bin_edges_[0].tolist()
            # 保存分箱数、策略和边界
            bin_rules[feature] = {
                'n_bins': self.config['continuous_features'][feature]['n_bins'],
                'strategy': self.config['continuous_features'][feature]['strategy'],
                'bin_edges': bin_edges  # 分箱边界列表，如 [10, 20, 30] 表示分箱为 (-inf,10], (10,20], (20,30], (30,+inf)
            }
        ...
        config_to_save['bin_rules'] = bin_rules  # 新增分箱规则字段
