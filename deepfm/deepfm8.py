...
class FeatureProcessor:
    ...
    def fit(self, df, categorical_features, continuous_features_config, bool_features):
        ...
        for feature in categorical_features:
            ...
            # 强制添加'NaN'到类别中（即使数据中没有缺失值）
            all_categories = df[feature].unique().tolist()
            if 'NaN' not in all_categories:
                all_categories.append('NaN')
        ...
        for feature in bool_features:
            le = LabelEncoder()
            # 处理缺失值：强制填充为False
            df[feature] = df[feature].fillna(False).astype(str)

            # 强制添加'False'（对应缺失值）到类别中
            all_categories = df[feature].unique().tolist()
            if 'False' not in all_categories:
                all_categories.append('False')
    ...
    def transform(self, df):
        ...
        for feature in self.config['categorical_features']:
            ...
            # 处理未见过的类别：全部映射为'NaN'
            known_classes = set(self.config['label_encoders'][feature].classes_)
            df[feature] = df[feature].apply(
                lambda x: x if x in known_classes else 'NaN'
            )

            # 转换（此时'NaN'一定在编码器中）
            encoded = self.config['label_encoders'][feature].transform(df[feature])
        ...
        for feature in self.config['continuous_features'].keys():
            ...
            # 加载分箱器和保存的分箱规则
            binner = self.config['bin_discretizers'][feature]
            saved_rules = self.config['bin_rules'].get(feature)

            # 校验分箱规则是否一致
            if saved_rules is None:
                raise ValueError(f"未找到特征 {feature} 的分箱规则，请先执行fit并保存配置")

            current_edges = binner.bin_edges_[0].tolist()
            if not np.allclose(current_edges, saved_rules['bin_edges'], atol=1e-6):
                raise ValueError(
                    f"分箱规则不一致！特征 {feature} 加载的分箱器边界与保存的规则不匹配。\n"
                    f"保存的边界: {saved_rules['bin_edges'][:5]}...\n"
                    f"分箱器实际边界: {current_edges[:5]}..."
                )

            # 处理缺失值
            df[feature] = df[feature].fillna(df[feature].median())
            # 转换为分箱索引
            binned = binner.transform(df[[feature]]).flatten()
        ...
        for feature in self.config['bool_features']:
            ...
            known_classes = set(self.config['label_encoders'][feature].classes_)
            df[feature] = df[feature].apply(
                lambda x: x if x in known_classes else 'False'
            )
...
def main():
    ...
    # 根据索引分割每个特征
    X_train = [feat[train_indices] for feat in X]
    X_val = [feat[val_indices] for feat in X]
