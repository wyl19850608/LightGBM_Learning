...
def add_days_from_today(df, date_column, new_column=None, handle_nulls=True):
    """
    给DataFrame添加一列，计算日期列中每个日期距离当天的天数，优化内存使用

    参数:
        df (pd.DataFrame): 包含日期列的DataFrame
        date_column (str): 日期列的列名
        new_column (str): 新列的列名，默认为None，此时将自动生成为"{date_column}_days_diff"
    ...
    # 计算天数差并转换为int32节省内存（天数差通常不需要64位）
...
def extract_user_features(df, user_id_col='user_id', label_col='label'):
    ...
    - 若存在label=1的记录，保留第一条label=1的记录
    - 若不存在label=1的记录，保留第一条label=0的记录
    ...
    def process_group(group):
        ...
        mask_1 = (labels == 1)
        if mask_1.any():
            # 直接返回第一条符合条件的索引标签（而非通过位置访问）
            return mask_1.idxmax()

        # 若无label=1，则查找第一条label=0的记录
        mask_0 = (labels == 0)
        if mask_0.any():
            # 直接返回第一条符合条件的索引标签
            return mask_0.idxmax()

        # 极端情况：既无0也无1，返回第一条记录的索引标签
        return group.index[0]  # 此处用[0]是位置索引，因group至少有一条记录
...
def save_preprocessors(encoders, binner,outlier_bounds,zscore_stats, save_dir):
    """保存编码器和分箱器到指定目录"""
    ...
    if outlier_bounds:
        outlier_path = os.path.join(encoders, CONFIG["preprocess_params"]["outlier_bounds_path"])
    with open(outlier_path, "wb") as f:
        pickle.dump(outlier_bounds, f)

    # 保存标准化统计量
    if zscore_stats:
        zscore_path = os.path.join(encoders, CONFIG["preprocess_params"]["zscore_stats_path"])
...
def load_data_from_folder(folder_path):
    ...
    combined_df = extract_user_features(combined_df)
    ...
    if numeric_features:
        ...
        with pd.option_context('display.max_columns', None, 'display.expand_frame_repr', False):
            print(combined_df[numeric_features].head(10))
    else:
...
def handle_missing_values(df, numeric_features, categorical_features):
    """根据配置处理缺失值，采用原地操作优化内存"""
    # 筛选出存在于DataFrame中的数值特征和类别特征，避免无效循环
    ...
    # 处理数值型特征缺失值（原地操作）
    if valid_numeric:
        strategy = CONFIG.get("numeric_missing_strategy", "default")
        # 预计算填充值（避免重复计算）
        fill_values = {}

        if strategy == "mean":
            fill_values = {col: df[col].mean() for col in valid_numeric if df[col].isnull().any()}
        elif strategy == "median":
            fill_values = {col: df[col].median() for col in valid_numeric if df[col].isnull().any()}
        else:
            default_val = CONFIG.get("numeric_fill_value", 0)
            fill_values = {col: default_val for col in valid_numeric if df[col].isnull().any()}

        # 原地填充缺失值（使用df.fillna的inplace参数）
        for col, val in fill_values.items():
            df.fillna({col: val}, inplace=True)

    # 处理类别型特征缺失值（原地操作）
    if valid_categorical:
        strategy = CONFIG.get("categorical_missing_strategy", "default")
        fill_values = {}

        if strategy == "mode":
            # 对每个类别特征计算众数（处理可能的多众数情况）
            for col in valid_categorical:
                if df[col].isnull().any():
                    modes = df[col].mode()
                    fill_values[col] = modes[0] if not modes.empty else CONFIG.get("categorical_fill_value", "Unknown")
        else:
            default_val = CONFIG.get("categorical_fill_value", "Unknown")
            fill_values = {col: default_val for col in valid_categorical if df[col].isnull().any()}
...
def save_results(trainer, config, test_metrics, encoders, binner, outlier_bounds, zscore_stats,results_dir=None):
    """将所有训练结果保存到文件"""
    ...
    save_preprocessors(encoders, binner, outlier_bounds, zscore_stats, results_dir)
...
class DeepFMDataset(Dataset):
    def __init__(self, df, numeric_features, categorical_features, label_col=None,
                 encoders=None, binner=None, outlier_bounds=None, zscore_stats=None, is_train=True):
        # 确保输入是DataFrame
        ...
        if self.is_train:
            ...
            if outlier_cfg["enabled"] and outlier_cfg["features"]:
                df, _, self.outlier_bounds = handle_outliers_by_iqr(
                    df=df,
...
class DeepFMTrainer:
    def __init__(self, sparse_feature_columns, dense_feature_columns,
                 embedding_dim=8, hidden_units=(128, 64, 32),
        ...
        """
        初始化DeepFM训练器
        :param sparse_feature_columns: 稀疏特征配置
        :param dense_feature_columns: 稠密特征配置
        :param 其他参数: 模型相关超参数
        """
        self.sparse_feature_columns = sparse_feature_columns
        self.dense_feature_columns = dense_feature_columns
        # self.feature_names = get_feature_names(sparse_feature_columns + dense_feature_columns)

        # 初始化deepctr_torch内置的DeepFM模型
        self.model = DeepFM(
    ...
    def train(self, x_train, y_train, x_val, y_val, batch_size=256, epochs=10):
        """训练模型"""
        self.model.fit(
            x_train, y_train,
    ...
    def save_model(self, model_path):
        ...
        save_dict = {
            ...
            'sparse_feature_columns': self.sparse_feature_columns,
            'dense_feature_columns': self.dense_feature_columns,
            'embedding_dim': self.model.embedding_dim,
            'hidden_units': self.model.dnn_hidden_units,
            'l2_reg_linear': self.model.l2_reg_linear,
            'l2_reg_embedding': self.model.l2_reg_embedding,
            'l2_reg_dnn': self.model.l2_reg_dnn,
            'init_std': self.model.init_std,
            'seed': self.model.seed,
            'dnn_dropout': self.model.dnn_dropout,
            'dnn_activation': self.model.dnn_activation,
    ...
    def load_model(cls, model_path):
        ...
        # 重建训练器实例
        trainer = cls(
            sparse_feature_columns=save_dict['sparse_feature_columns'],
            dense_feature_columns=save_dict['dense_feature_columns'],
            embedding_dim=save_dict['embedding_dim'],
            hidden_units=save_dict['hidden_units'],
            l2_reg_linear=save_dict['l2_reg_linear'],
            l2_reg_embedding=save_dict['l2_reg_embedding'],
            l2_reg_dnn=save_dict['l2_reg_dnn'],
            init_std=save_dict['init_std'],
            seed=save_dict['seed'],
            dnn_dropout=save_dict['dnn_dropout'],
            dnn_activation=save_dict['dnn_activation'],
            task=save_dict['task'],
...
def train_model(data_folder, test_folder,results_dir=None):
    ...
    outlier_bounds = train_dataset.outlier_bounds
    zscore_stats = train_dataset.zscore_stats
    ...
    trainer = DeepFMTrainer(
        sparse_feature_columns=len(CONFIG["numeric_features"]),
        dense_feature_columns=len(CONFIG["categorical_features"]),
    ...
    # 8. 模型评估
    print("在测试集上评估模型...")
    test_metrics, y_true, y_pred = trainer.evaluate(test_loader)
    trainer.test_confusion_matrix = confusion_matrix(y_true, (np.array(y_pred) >= trainer.best_threshold).astype(int))


    # 保存结果
    # results_dir = save_results(trainer, CONFIG, test_metrics, encoders, binner, outlier_bounds, zscore_stats, results_dir)
    return results_dir
...
if __name__ == "__main__":
    ...
    results_dir = train_model("./","./test_data")  # 取消注释此、行进行训练
