...
class DeepFM(nn.Module):
    def __init__(self, numeric_dim, categorical_dims, embed_dim=32, mlp_layers=[256, 128, 64], dropout=0.3):
        ...
        # 二阶部分 - FM交互
        self.fm_interaction = lambda x: 0.5 * (
                torch.pow(torch.sum(x, dim=1), 2) - torch.sum(torch.pow(x, 2), dim=1)
        ).sum(1, keepdim=True)
    ...
    def forward(self, numeric, categorical):
        ...
        for i, emb in enumerate(self.embeddings):
            # 使用嵌入的第一个维度作为一阶特征
            first_order_cats.append(emb(categorical[:, i])[:, 0].unsqueeze(1))
        first_order_cats = torch.cat(first_order_cats, dim=1)
        first_order_features = torch.cat([numeric, first_order_cats], dim=1)
        first_order_out = self.first_order(first_order_features)

        # 二阶特征交互
        embeddings = []
        for i, emb in enumerate(self.embeddings):
            embeddings.append(emb(categorical[:, i]))
        embeddings = torch.stack(embeddings, dim=1)  # [batch_size, num_cats, embed_dim]
        fm_out = self.fm_interaction(embeddings)

        # 深度部分
        flat_embeddings = embeddings.view(embeddings.size(0), -1)  # 展平嵌入
        deep_input = torch.cat([numeric, flat_embeddings], dim=1)
        deep_out = self.mlp(deep_input)

        # 合并所有部分
        total = first_order_out + fm_out + deep_out
        return self.output(total).squeeze(1)
