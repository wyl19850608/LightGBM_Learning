...
class ContinuousFeatureBinner:
    """连续特征分箱处理器，支持多种分箱方法并可保存配置"""

    def __init__(self):
        self.binning_config = {}  # 存储分箱配置
        self.features = []        # 分箱特征列表

    def fit(self, df, features, target=None, method='equal_width', n_bins=5,
        ...
        参数:
            ...
            method: 分箱方法
                   ...
                   - 'kmeans': K-means聚类分箱
                   - 'chi2': 卡方分箱（有监督）
                   - 'decision_tree': 决策树分箱（有监督）
            n_bins: 分箱数量
            min_samples_leaf: 决策树分箱时的最小叶子节点样本比例
            random_state: 随机种子
        """
        self.features = features
        self.binning_config['method'] = method
        self.binning_config['n_bins'] = n_bins
        ...
        for feature in features:
            ...
            # 决策树分箱（有监督）
                ...
                if target is None:
                    raise ValueError("使用决策树分箱需要指定目标变量target")
            ...
            # 卡方分箱（有监督）
                ...
                if target is None:
                    raise ValueError("使用卡方分箱需要指定目标变量target")
    ...
    def load_config(self, file_path):
        ...
        # 提取特征列表
        self.features = [f for f in self.binning_config.keys() if f not in ['method', 'n_bins']]
...
USER_000007,23.1,6154.23,9.42,7.96,Female,Student,San Antonio,Bachelor Degree,4,8,3,4,1,False,False,False,1"""
    ...
    # 定义连续特征
    continuous_features = ['age', 'monthly_income', 'browse_time_minutes', 'spending_frequency']
    ...
    # 对不同特征使用不同分箱方法
    print("对age使用等宽分箱...")
    binner_age = ContinuousFeatureBinner()
    binner_age.fit(df, ['age'], method='equal_width', n_bins=3)
    binner_age.save_config('binning_configs/age_binning.json')

    print("\n对monthly_income使用等频分箱...")
    binner_income = ContinuousFeatureBinner()
    binner_income.fit(df, ['monthly_income'], method='equal_freq', n_bins=4)
    binner_income.save_config('binning_configs/income_binning.json')

    print("\n对browse_time_minutes使用K-means聚类分箱...")
    binner_browse = ContinuousFeatureBinner()
    binner_browse.fit(df, ['browse_time_minutes'], method='kmeans', n_bins=3)
    binner_browse.save_config('binning_configs/browse_time_binning.json')

    print("\n对spending_frequency使用决策树分箱（有监督）...")
    binner_spend = ContinuousFeatureBinner()
    binner_spend.fit(df, ['spending_frequency'], target='label', method='decision_tree', n_bins=3)
    binner_spend.save_config('binning_configs/spending_binning.json')

    # 加载配置并转换新数据
    print("\n加载分箱配置并转换数据...")
    new_binner = ContinuousFeatureBinner()
    new_binner.load_config('binning_configs/age_binning.json')
    df_transformed = new_binner.transform(df)

    print("\n转换后的年龄分箱结果:")
    print(df_transformed[['age', 'age_binned']])

    # 查看分箱统计信息
    print("\n年龄分箱统计信息:")
    import pprint
    pprint.pprint(new_binner.binning_config['age']['stats'])
