...
class ContinuousFeatureBinner:
    """连续特征分箱处理器，支持为每个特征指定不同分箱方法、分箱数量及缺失值替换值"""
    ...
    def fit(self, df, features, target=None, min_samples_leaf=0.05, random_state=42):
        """
        训练分箱规则，支持为每个特征单独指定分箱方法、数量及缺失值替换值

        参数:
            df: 包含特征的数据框
            features: 字典形式，键为特征名，值为包含分箱参数的字典
                      例如: {'age': {'method': 'equal_width', 'n_bins': 3, 'replace_na': -1},
                             'income': {'method': 'kmeans', 'n_bins': 5, 'replace_na': 999}}
        ...
        for feature, params in features.items():
            ...
            # 获取当前特征的缺失值替换值，默认为-1
            replace_na = params.get('replace_na', -1)
            ...
            self.binning_config[feature] = {
                ...
                'replace_na': replace_na  # 保存缺失值替换配置
            }
    ...
    def transform(self, df, replace_na=None):
        """
        应用分箱规则转换数据

        参数:
            df: 需要转换的数据框
            replace_na: 全局缺失值替换值，若为None则使用每个特征在fit时配置的replace_na
        """
        ...
        for feature in self.features:
            ...
            # 确定当前特征的缺失值替换值：优先使用fit时配置的值，其次提供则使用全局replace_na
            feature_replace_na = self.binning_config[feature]['replace_na']
            if replace_na is not None:
                feature_replace_na = replace_na
            ...
            if df_transformed[feature].isna().any():
                df_transformed[f'{feature}_binned'] = df_transformed[f'{feature}_binned'].cat.add_categories(feature_replace_na)
                df_transformed[f'{feature}_binned'].fillna(feature_replace_na, inplace=True)
...
USER_000007,23.1,6154.23,9.42,7.96,Female,Student,San Antonio,Bachelor Degree,4,8,3,4,1,False,False,False,1"""
    ...
    # 定义每个特征的分箱配置，包括各自的缺失值替换值
    feature_configs = {
        'age': {'method': 'equal_width', 'n_bins': 3, 'replace_na': -1},  # 年龄缺失用-1表示
        'monthly_income': {'method': 'equal_freq', 'n_bins': 4, 'replace_na': -999},  # 月收入缺失用-999表示
        'browse_time_minutes': {'method': 'kmeans', 'n_bins': 3, 'replace_na': -99},  # 浏览时间缺失用-99表示
        'spending_frequency': {'method': 'decision_tree', 'n_bins': 3}  # 未指定replace_na，使用默认值-1
    }
    ...
    for feature in feature_configs:
        print(f"{feature}: {binner.binning_config[feature]['replace_na']}")
