        ...
        for feature, bin_strategies in features_config.items():
            ...
            for strategy in bin_strategies:
                ...
                # 决策树分箱（兼容所有Spark版本的实现）
                    ...
                    # 兼容所有Spark版本的阈值提取方法（修正后）
                    thresholds = []
                    try:
                        # 方法1: 尝试新版本API（Spark 3.x+）
                        nodes = model.treeNodeCollector().collect()
                        for node in nodes:
                            if node.split:  # 检查是否有分裂（非叶节点）
                                thresholds.append(float(node.split.threshold))
                    except AttributeError:
                        try:
                            # 方法2: 尝试旧版本API - nodeAt（Spark 2.x）
                            java_model = model._java_obj
                            node_count = java_model.numNodes()
                            for i in range(node_count):
                                node = java_model.nodeAt(i)
                                if not node.isLeaf:  # isLeaf是属性，无括号
                                    thresholds.append(float(node.split().threshold()))
                        except Exception:
                            try:
                                # 方法3: 递归访问根节点（兼容更多版本）
                                java_model = model._java_obj
                                tree = java_model.rootNode()  # 获取根节点

                                # 递归提取阈值
                                def extract_thresholds(node, thresholds_list):
                                    if not node.isLeaf:  # 非叶节点
                                        thresholds_list.append(float(node.split().threshold()))
                                        extract_thresholds(node.leftChild(), thresholds_list)
                                        extract_thresholds(node.rightChild(), thresholds_list)
                ...
                # 卡方分箱
                    ...
                    unique_values = [row[feature] for row in clean_data.select(feature).distinct().orderBy(feature).collect()]
        ...
        for feature in self.features:
            ...
            for bin_id, bin_config in self.binning_config[feature].items():
                ...
                df_transformed = df_transformed.withColumn(
                    new_column,
                    when(
                        col(f"{feature}_temp_bin").isNull() | isnan(col(f"{feature}_temp_bin")),
                        lit(replace_na)
