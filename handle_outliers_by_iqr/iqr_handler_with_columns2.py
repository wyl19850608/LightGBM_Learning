...
if __name__ == "__main__":
    ...
    data = {
        ...
        "age": [25, 30, 150, -5, 40, 45, 50, 55, 60, 65, 70, 75]
    }
    ...
    # 1. 使用IQR方法处理异常值
    print("\n===== 使用IQR方法处理 =====")
    processed_df, handled_cols, iqr_fill_values = handle_outliers_by_iqr(
        df,
        features=["values", "score"],
        new_column_suffix="_iqr"
    )

    # 2. 使用指定上下界处理异常值（例如对年龄进行处理，合理范围0-120岁）
    print("\n===== 使用指定上下界处理 =====")
    bounds = {
        "age": (0, 120),  # 年龄合理范围0-120岁
        "score": (0, 200)  # 分数合理范围0-200分
    }
    processed_df, handled_cols, bound_fill_values = handle_outliers_by_bounds(
        processed_df,
        bounds=bounds,
        new_column_suffix="_bounded"
    )
    ...
    print("\nIQR方法填充值:")
    for feat, bounds in iqr_fill_values.items():
        print(f"  {feat}: 下界={bounds['lower_bound']:.2f}, 上界={bounds['upper_bound']:.2f}")
    print("\n指定上下界填充值:")
    for feat, bounds in bound_fill_values.items():
