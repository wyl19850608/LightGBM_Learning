...
from data_analysis.corr_df import spark
...
def load_training_data(file_path="training_data.csv"):
    ...
    print(f"成功加载训练数据，共{len(df)}行，{len(df.columns)}列特征")
    # print(f"正样本比例: {df['label'].mean():.4f}")

    return df

# 训练模型 - 修改为使用加载的特征数据
def train_lightgbm_model(model_path, meta_path, data_path="training_data.csv"):
    # 加载自定义特征数据
    ...
    for feat in continuous_features:
        # 空字符串转为NaN，再填充-1
        df[feat] = pd.to_numeric(df[feat], errors='coerce').fillna(-1.0)

    df['label'] = ((df['is_coupon_used'] == "1") & (df['wdraw_amt'] > 0)).astype(int)
    print(f"正样本比例: {df['label'].mean():.4f}")
    ...
    for feat in cat_features:
        # 确保特征是字符串类型再进行编码
        df[feat] = df[feat].astype(str).astype('category').cat.codes
    ...
    X_train = X_train.drop(columns=['user_id', 'unique_id'])
    X_val = X_val.drop(columns=['user_id', 'unique_id'])
    ...
    cat_info = {}
    # 重新加载原始数据以获取未编码的类别信息
    original_df = pd.read_csv(data_path, encoding='utf-8', usecols=cat_features)
    for feat in cat_features:
        # 确保特征是字符串类型
...
def main():
    ...
    try:
        print("开始训练模型...")
        # 训练模型并保存特征信息
        cat_features, num_features, cat_info = train_lightgbm_model(model_path, meta_path, data_path)
        ...
        test_df = load_training_data("training_data.csv")
        for feat in continuous_features:
            test_df[feat] = pd.to_numeric(test_df[feat], errors='coerce').fillna(-1.0)

        # test_df = test_df.drop(columns=['user_id', 'unique_id'])
