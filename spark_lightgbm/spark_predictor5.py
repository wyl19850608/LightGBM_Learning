...
def generate_training_data(sample_size=100000):
    ...
    # 转换为DataFrame并确保使用原生Python类型
    df = pd.DataFrame(X, columns=[f'num_feat_{i}' for i in range(15)]).applymap(float)
...
def generate_test_data(spark, feature_names, cat_info, sample_size=20000):
    ...
    # 生成数值特征并转换为原生Python类型
    for feat in num_features:
        # 生成numpy数组后立即转换为Python列表
        data[feat] = np.random.normal(loc=0.1, scale=1.2, size=sample_size).tolist()
    ...
    # 转换为pandas DataFrame，确保所有数值都是原生类型
...
class LightGBMPredictor:
    ...
    def predict(self, spark_df):
        """使用applyInPandas API进行预测，确保所有数据类型都是原生Python类型"""
        # 广播模型和分类信息到所有节点
        ...
        def predict_batch(pdf_iter):
            ...
            for pdf in pdf_iter:
                ...
                # 提取ID和特征，确保所有数据都是原生Python类型
                ids = pdf['id'].tolist()  # 直接转换为Python列表
                ...
                for feat in cat_features:
                    ...
                    cat_map['unknown'] = -1
                    # 转换为原生int类型
                    X_dict[feat] = [cat_map.get(x, -1) for x in pdf[feat].tolist()]

                # 处理数值特征 - 确保是原生Python float
                for feat in num_features:
                    # 先转换为列表，再逐个转换为Python float
                    X_dict[feat] = [float(x) if not pd.isna(x) else 0.0 for x in pdf[feat].tolist()]
                ...
                predictions = [int(p > 0.5) for p in probabilities]  # 确保是Python int
